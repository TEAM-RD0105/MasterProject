...
#define DSPI_IP_TYPES_SW_MAJOR_VERSION               6
...
typedef enum
{
    ...
    DSPI_IP_EVENT_COMPARE_MATCH = 2  /**< Deserialized data match the comparisons masks */
#endif
} Dspi_Ip_EventType;

/** @brief   Callback for all peripherals which supports SPI features. */
typedef void (*Dspi_Ip_CallbackType)(uint8 Instance,
...
typedef struct
{
    ...
    #if (DSPI_IP_SERIALIZATION_SUPPORT == STD_ON)
    boolean SerializationEnabled; /* True if serialization is configured */
    boolean EnableCommandFrames; /* Enable to send command (normal SPI frames) during serialization */
    uint16 DataFrameSize; /* Framesize of a data frame0 */
    uint16 DataFrame1Size; /* Framesize of a data frame1 */
    uint32 DSICR0; /* Configuration register 0 */
    uint32 DSICR1; /* Configuration register 1 */
    uint64 SSR; /* Value to write to Source select register */
    uint64 DIMR; /* Interrupt mask register */
    uint64 DPIR; /* Interrupt polarity register */
    uint32 TSL;  /* Time slots times */
    uint32 TS_CONF; /* Time slots configurations */
    boolean StopOnInterrupt; /* Stop on DSI frame match */
    Dspi_Ip_CallbackType SerializationNotification; /* Called with DSPI_IP_EVENT_COMPARE_MATCH */       
    #endif
...
typedef struct
{
    ...
    uint16 CurrentTxFifoSlot; /**< Number of TX FIFO slots are current available. */ uint16 Pushr0Repeat;     /**< Number of times to write
                                                                                                               * PushrCmds[0] to FIFO >*/
...
typedef struct
{
    ...
    boolean KeepCs;
    Dspi_Ip_DeviceParamsType *DeviceParams; /**< Device params to be updated. If null, device params are not updated */
} Dspi_Ip_TransferAdjustmentType;
