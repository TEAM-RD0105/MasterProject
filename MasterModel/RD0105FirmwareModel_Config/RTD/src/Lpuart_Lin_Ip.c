...
#define LPUART_LIN_IP_SW_MAJOR_VERSION_C               6
...
/* Check if current file and Lpuart_Lin_Ip header file are of the same Software version */
    ...
    #if (LPUART_LIN_IP_HAS_DMA_ENABLED == STD_ON)
        #if ((LPUART_LIN_IP_AR_RELEASE_MAJOR_VERSION_C != DMA_IP_AR_RELEASE_MAJOR_VERSION) || \
             (LPUART_LIN_IP_AR_RELEASE_MINOR_VERSION_C != DMA_IP_AR_RELEASE_MINOR_VERSION))
            #error "AutoSar Version Numbers of Lpuart_Lin_Ip.c and Dma_Ip.h are different"
        #endif
    #endif
...
#if (LPUART_LIN_IP_HAS_DMA_ENABLED == STD_ON)
    #define LPUART_LIN_IP_DMA_CONFIG_LIST_DIMENSION             (10U)
    #define LPUART_LIN_IP_DMA_LEAST_CONFIG_LIST_DIMENSION       (2U)
    /* @brief Address of the least significant byte or word in a 32-bit register (depends on endianness) */
    #define LPUART_LIN_IP_LSBW_ADDR(reg)  ((uint32)(&(reg)))
#endif
...
static void Lpuart_Lin_Ip_HwSetTransceiverCmd(const uint8 Instance, boolean Enable);
...
#if (LPUART_LIN_IP_HAS_DMA_ENABLED == STD_ON)
static Lpuart_Lin_Ip_StatusType Lpuart_Lin_Ip_StartReceiveResponseUsingDma(const uint8 Instance,
                                                                         uint8 * RxBuff,
                                                                         const uint32 RxSize
                                                                          );
static void Lpuart_Lin_Ip_CompleteRxOperationUsingDma(const uint8 Instance);
#endif
/*==================================================================================================
*                                         LOCAL FUNCTIONS
==================================================================================================*/
...
static void Lpuart_Lin_Ip_CompleteRxOperationUsingDma(const uint8 Instance)
{
    ...
    const Lpuart_Lin_Ip_UserConfigType *LinUserCfg;
    LPUART_Type * Base;
    uint8 BuffSize = 0;
    ...
    LinCurrentState = (Lpuart_Lin_Ip_StateStructType *)Lpuart_Lin_Ip_apxStateStructureArray[Instance];
    LinUserCfg = Lpuart_Lin_Ip_apxUserConfigs[Instance];

    /* Disable rx DMA requests for the current instance*/
    Lpuart_Lin_Ip_SetRxDmaCmd(Base, FALSE);

    /* Read dummy to clear RDRF flag */
    (void)Lpuart_Lin_Ip_HwGetchar(Base, &TmpByte);

    /* Update the information of the module driver state */
    LinCurrentState->IsBusBusy = FALSE;

    if (LPUART_LIN_IP_STATUS_DMA_ERROR != LinCurrentState->CurrentEventId)
    {
        BuffSize = LinCurrentState->RxSize;
        ...
        LinCurrentState->Checksum = Lin_Ip_MakeChecksumByte(&Lpuart_Lin_Ip_au8SduBuffer[Instance][0u], BuffSize - 1, CurrentChecksum);

        if (Lpuart_Lin_Ip_au8SduBuffer[Instance][BuffSize - 1] != LinCurrentState->Checksum)
        {
    ...
    if (LinUserCfg->Callback != NULL_PTR)
    {
        LinUserCfg->Callback(Instance, LinCurrentState);
    }
}
/*FUNCTION**********************************************************************
 *
 * Function Name : Lpuart_Lin_Ip_StartReceiveResponseUsingDma
 * Description   : Initiate (start) a receive by beginning the process of
 * receiving response using DMA transfers.
 * This is not a public API as it is called from other driver functions.
 *
 *END**************************************************************************/
static Lpuart_Lin_Ip_StatusType Lpuart_Lin_Ip_StartReceiveResponseUsingDma(const uint8 Instance, uint8 * RxBuff, const uint32 RxSize)
{
    Lpuart_Lin_Ip_StateStructType * LinCurrentState;
    const Lpuart_Lin_Ip_UserConfigType *LinUserCfg;
    LPUART_Type * Base;
    Dma_Ip_LogicChannelTransferListType DmaTransferList[LPUART_LIN_IP_DMA_CONFIG_LIST_DIMENSION];
    ...
    Lpuart_Lin_Ip_ClearErrorFlags(Base);
    /* Flush the Rx Buffer */
    Lpuart_Lin_Ip_FlushRxBuffer(Base);

    /* Set up parameters for Dma_Ip_LogicChannelTransferListType */
    DmaTransferList[0].Param = DMA_IP_CH_SET_SOURCE_ADDRESS;
    DmaTransferList[0].Value = LPUART_LIN_IP_LSBW_ADDR(Base->DATA);
    ...
    DmaReturnStatus = Dma_Ip_SetLogicChannelTransferList(LinUserCfg->RxDMAChannel, DmaTransferList, LPUART_LIN_IP_DMA_CONFIG_LIST_DIMENSION);
#if (LPUART_LIN_IP_DEV_ERROR_DETECT == STD_ON)
    LPUART_LIN_IP_DEV_ASSERT(DMA_IP_STATUS_SUCCESS == DmaReturnStatus);
#endif
    /* Start the DMA channel */
    DmaReturnStatus = Dma_Ip_SetLogicChannelCommand(LinUserCfg->RxDMAChannel, DMA_IP_CH_SET_HARDWARE_REQUEST);
#if (LPUART_LIN_IP_DEV_ERROR_DETECT == STD_ON)    
    LPUART_LIN_IP_DEV_ASSERT(DMA_IP_STATUS_SUCCESS == DmaReturnStatus);
#endif
    (void)DmaReturnStatus;
    /* Enable rx DMA requests for the current instance */
    Lpuart_Lin_Ip_SetRxDmaCmd(Base, TRUE);
...
static void Lpuart_Lin_Ip_StartSendFrame(const uint8 Instance, const Lpuart_Lin_Ip_PduType *PduInfoPtr)
{
    ...
    else
    {
        ...
        {
#if (LPUART_LIN_IP_HAS_DMA_ENABLED == STD_ON)
            if (UserConfig->TransferType == LPUART_LIN_IP_USING_DMA)
            {
                Lpuart_Lin_Ip_HwSetIntMode(Base, LPUART_LIN_IP_INT_RX_DATA_REG_FULL, FALSE);
                Lpuart_Lin_Ip_StartReceiveResponseUsingDma(Instance, Lpuart_Lin_Ip_au8SduBuffer[Instance], LinCurrentState->RxSize);
            }
#endif
        #if (LPUART_LIN_IP_FRAME_TIMEOUT_DISABLE == STD_OFF)
...
static void Lpuart_Lin_Ip_ProcessBreakDetect(const uint8 Instance)
{
    ...
    /* Clear Rx complete interrupt flag by flushing the Rx Buffer */
    Lpuart_Lin_Ip_FlushRxBuffer(Base);
...
static void Lpuart_Lin_Ip_HwSetTransceiverCmd(const uint8 Instance, boolean Enable)
{
    ...
    Lpuart_Lin_Ip_HwSetTransmitterCmd(Base, Enable);
    Lpuart_Lin_Ip_HwSetReceiverCmd(Base, Enable);
}
/*FUNCTION**********************************************************************
 *
...
Lpuart_Lin_Ip_StatusType Lpuart_Lin_Ip_Init(const uint8 Instance, const Lpuart_Lin_Ip_UserConfigType * UserConfig)
{
    ...
    */
    ...
    if (TRUE == CrtStateStruct->BaudrateEvalEnable)
    {
        ...
        Lpuart_Lin_Ip_HwSetTransceiverCmd(Instance, FALSE);
    }
    else
#endif /* (LPUART_LIN_IP_AUTO_BAUD == STD_ON) */
    {
        /* Enable the LPUART transmitter and receiver */
        Lpuart_Lin_Ip_HwSetTransceiverCmd(Instance, TRUE);
    }
    (void)(RetStatus);
...
/*FUNCTION**********************************************************************
 *
 * Function Name : Lpuart_Lin_Ip_CompleteReceiveUsingDma
 ...
 *END**************************************************************************/
/**
* @implements Lpuart_Lin_Ip_CompleteReceiveUsingDma_Activity
*/
void Lpuart_Lin_Ip_CompleteReceiveUsingDma(uint8 Instance)
{
    Lpuart_Lin_Ip_StateStructType *LinCurrentState;
    const Lpuart_Lin_Ip_UserConfigType *UserConfig;
    Dma_Ip_ReturnType DmaReturnStatus;
    ...
    (void)Dma_Ip_GetLogicChannelStatus(UserConfig->RxDMAChannel, &DmaStatus);

    if (DMA_IP_CH_ERROR_STATE == DmaStatus.ChStateValue)
    {
        /* Reset the Dma Channel Error status. */
        DmaReturnStatus = Dma_Ip_SetLogicChannelCommand(UserConfig->RxDMAChannel, DMA_IP_CH_CLEAR_ERROR);
#if (LPUART_LIN_IP_DEV_ERROR_DETECT == STD_ON)
        LPUART_LIN_IP_DEV_ASSERT(DMA_IP_STATUS_SUCCESS == DmaReturnStatus);
#endif
        (void)DmaReturnStatus;
        /* Update event id */
        LinCurrentState->CurrentEventId = LPUART_LIN_IP_STATUS_DMA_ERROR;
    }
    /* Operation for complete receive data */
    Lpuart_Lin_Ip_CompleteRxOperationUsingDma(Instance);
}
#endif
/*FUNCTION**********************************************************************
 *
    ...
    switch (LinCurrentState->CurrentNodeState)
    {
        ...
        case LPUART_LIN_IP_NODE_STATE_RECV_DATA_COMPLETED:
            ...
            if (UserConfig->TransferType == LPUART_LIN_IP_USING_DMA)
            {
                ...
                */
                (void)Dma_Ip_GetLogicChannelParam(UserConfig->RxDMAChannel, DmaLogicChnParam, (uint32 *)&RemainingByte);
                /* CntByte is used in Lpuart_Lin_Ip_GetStatusFromTimeoutError function to report error */
                LinCurrentState->CntByte = LinCurrentState->RxSize - RemainingByte;
            }
#endif
...
void Lpuart_Lin_Ip_GoToIdleState(const uint8 Instance)
{
    ...
    /* Before enable Rx overrun, we need to discard data in Rx buffer, to avoid the next time receive data will raise overrun error */
    /* Switching to idle state also means that the data in the receive buffer is no longer meaningful. */
    /* Flush the Rx Buffer */
    Lpuart_Lin_Ip_FlushRxBuffer(Base);
