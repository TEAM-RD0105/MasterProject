...
#define SPI_IPW_SW_MAJOR_VERSION_C                6
...
static boolean Spi_Ipw_AreDeviceParamsCompatible
...
(
    ...
    const Spi_HalfDuplexModeType TransferType
);
#else
(
    const Lpspi_Ip_ExternalDeviceType *ExternalDevice,
...
/**
 * @brief Validates the compatibility of device parameters with channel configuration
 *
 * @param[in] ExternalDevice            Pointer to the external device configuration
 * @param[in] ChannelConfig             Pointer to the channel configuration
 * @param[in] TransferType              Type of transfer (full/half duplex)
 *
                                                 ...
                                                 const Spi_Ipw_ParamDMACont *Param)
{
    ...
    static boolean IsRxBufferNull = FALSE;
#if (STD_ON == SPI_IPW_SPI_HALF_DUPLEX_MODE_SUPPORT)
    static Spi_HalfDuplexModeType TransferType = SPI_FULL_DUPLEX;
#endif
        ...
        if (EB == ChannelConfig->BufferType)
        {
            ...
            if (Param->IsFirstJob && (0U == Param->ChannelIndex))
            {
                ...
                *(Param->ComparedRxBufferAddr) = RxBufferAddr;
                ...
                ExternalDevice->DeviceParams->Lsb = ChannelConfig->Lsb;
                ExternalDevice->DeviceParams->ByteSwap = ChannelConfig->ByteSwap;
                if(NULL_PTR == (uint32*)RxBufferAddr)
                {
                    IsRxBufferNull = TRUE;
                }
                else
                {
                    IsRxBufferNull = FALSE;
                }
#if (STD_ON == SPI_IPW_SPI_HALF_DUPLEX_MODE_SUPPORT)
                TransferType = ChannelConfig->HalfDuplexMode;
                if (SPI_HALF_DUPLEX_TRANSMIT == TransferType)
                {
                    SeqTxBufferList->TransferType = LPSPI_IP_HALF_DUPLEX_TRANSMIT;
                }
                if (SPI_HALF_DUPLEX_RECEIVE == TransferType)
                {
                    SeqTxBufferList->TransferType = LPSPI_IP_HALF_DUPLEX_RECEIVE;
                }
#endif
            }
            else
            {
                if ((((*(Param->ComparedRxBufferAddr) != RxBufferAddr) && !IsRxBufferNull)
#if (STD_ON == SPI_IPW_SPI_HALF_DUPLEX_MODE_SUPPORT)
                        && (SPI_HALF_DUPLEX_TRANSMIT != TransferType) 
#endif              
                    ) ||
                    (IsRxBufferNull && (NULL_PTR != (uint32*)RxBufferAddr)) ||
                    (ExternalDevice != Param->FirstExternalDeviceConfig->LpspiExternalDeviceConfig) ||
#if (STD_ON == SPI_IPW_SPI_HALF_DUPLEX_MODE_SUPPORT)
                    (Spi_Ipw_AreDeviceParamsCompatible(ExternalDevice, ChannelConfig, TransferType) == FALSE)
#else
                    (Spi_Ipw_AreDeviceParamsCompatible(ExternalDevice, ChannelConfig) == FALSE)
#endif        
                    )
                {
                    Ret = E_NOT_OK;
