...
#define SPI_SW_MAJOR_VERSION_C              6
...
typedef struct
{
    volatile uint32 Busy;           /**< @brief Spi Sync Transmit Running HWUnits Status. */
    ...
    boolean SerializationInProgress; /**< @brief Serialization in progress */
    Spi_ExternalDeviceType DevInUse; /**< @brief External device used for serialization */
#endif
} Spi_HWStateType;
...
#if (SPI_SERIALIZATION_SUPPORT == STD_ON)
        if (ChannelConfig->IsSerialized)
        {
            Spi_Ipw_TransmitSerializedData(JobConfig->ExternalDeviceConfig->ExDeviceConfig,
                                           TxBuffer, RxBuffer);
        }
        else
#endif
                                          ...
                                          uint32 SpiID)
{
    ...
    while (Job < SequenceConfig->NumJobs)
    {
        ...
        /* Mark HWUnit as busy */
        if (SPI_IDLE == Spi_axSpiHwUnitQueueArray[JobConfig->HWUnit].Status)
        {
...
#if (SPI_SERIALIZATION_SUPPORT == STD_ON)
        else if (((boolean)TRUE == ChannelConfig->IsSerialized) && (Length != BytesPerFrame))
        {
            /* Serialized buffers only support one frame */
            Status = (Std_ReturnType)E_NOT_OK;
        }
        else
        {
...
#if (SPI_SERIALIZATION_SUPPORT == STD_ON)
        else if (((boolean)TRUE == ChannelConfig->IsSerialized) && (Length != 1u))
        {
            /* Serialized buffers only support one frame  */
            Status = (Std_ReturnType)E_NOT_OK;
        }
...
#if (SPI_SERIALIZATION_SUPPORT == STD_ON)
    else if (SPI_RET_NOT_OK == Spi_SerializationTransmitCheck(Spi_apxSpiConfigPtr[SpiID],
                                                              Spi_apxSpiConfigPtr[SpiID]->SequenceConfig[Sequence].SeqConfig))
    {
        (void)Det_ReportError((uint16) SPI_MODULE_ID,(uint8) 0, SPI_ASYNCTRANSMIT_ID, SPI_E_SERIALIZATION_WRONG_DEV);
    }
#endif
    else
    {
                                                               ...
                                                               const Spi_JobConfigType *JobConfig)
{
    ...
    {
        ChannelID = JobConfig->ChannelIndexList[ChannelIndex];
        ChannelCfg = Spi_apxSpiConfigPtr[SpiID]->ChannelConfig[ChannelID].ChannelCfg;

#if (SPI_SERIALIZATION_SUPPORT == STD_ON)
        if ((boolean)TRUE == ChannelCfg->IsSerialized)
        {
            /* Serialized channels are not supported for asynchronous transmissions */
            (void)Det_ReportError((uint16) SPI_MODULE_ID,(uint8) 0,SPI_ASYNCTRANSMIT_ID, SPI_E_PARAM_CONFIG);
            Status = (Std_ReturnType)E_NOT_OK;
        }
        else
#endif
        if (EB == ChannelCfg->BufferType)
        {
                                                     ...
                                                     Spi_SequenceType Sequence)
{
    ...
    else if (SpiID != Spi_apxSpiConfigPtr[SpiID]->SequenceConfig[Sequence].SeqConfig->SpiCorePartitionUse)
    {
        (void)Det_ReportError((uint16) SPI_MODULE_ID,(uint8) 0,SPI_SYNCTRANSMIT_ID,SPI_E_PARAM_CONFIG);
                                                              ...
                                                              Spi_apxSpiConfigPtr[SpiID]->SequenceConfig[Sequence].SeqConfig))
    {
        (void)Det_ReportError((uint16) SPI_MODULE_ID,(uint8) 0,SPI_SYNCTRANSMIT_ID, SPI_E_SERIALIZATION_WRONG_DEV);
    }
#endif
    else
    {
...
Spi_StatusType Spi_GetStatus(void)
{
    ...
    else
    {
        ...
        for (HWUnit = 0u; HWUnit < (Spi_HWUnitType)SPI_MAX_HWUNIT; HWUnit++)
        {
            ...
            if ((NULL_PTR != PhyUnitConfig) && (SpiID == PhyUnitConfig->SpiCorePartitionUse))
            {
                if (1u == Spi_axHwUnitState[HWUnit].Busy)
                {
                    StatusFlag = SPI_BUSY;
                    break;
                }
                                 ...
                                 uint32 Busy)
{
    ...
    for (JobIndex = 0u; JobIndex < SequenceConfig->NumJobs; JobIndex++)
    {
        ...
        Spi_axHwUnitState[HWUnit].Busy = Busy;
#if ((SPI_LEVEL_DELIVERED == SPI_LEVEL1) || (SPI_LEVEL_DELIVERED == SPI_LEVEL2))
        if (1u == Busy)
        {
...
Std_ReturnType Spi_SyncTransmit(Spi_SequenceType Sequence)
{
    ...
    if ((Std_ReturnType)E_NOT_OK != Status)
    {
        ...
        for (HWUnit = 0u; HWUnit < (Spi_HWUnitType)SPI_MAX_HWUNIT; HWUnit++)
        {
            if ((0u != Spi_axHwUnitState[HWUnit].Busy) &&
