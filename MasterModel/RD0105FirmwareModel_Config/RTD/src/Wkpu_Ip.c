...
#define WKPU_IP_SW_MAJOR_VERSION_C             6
 ...
 *
 * @param[in] base The WKPU peripheral base address
 * @param[in] coreNumber NMI sources
...
static inline boolean Wkpu_Ip_IsNMIConfigLock(const WKPU_Type * base, uint8 coreNumber)
{
    ...
    uint32 coreShift;
        ...
        coreShift = (uint32)coreNumber * WKPU_IP_CORE_OFFSET_SIZE;
...
Wkpu_Ip_StatusType Wkpu_Ip_DeinitNMI(uint8 instance)
{
    ...
    {
        if (Wkpu_Ip_IsNMIConfigLock(base, (uint8)coreNumber[i]) == FALSE)
        {
