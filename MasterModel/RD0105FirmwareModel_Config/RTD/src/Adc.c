...
#define ADC_SW_MAJOR_VERSION_C                  6
...
#if (((ADC_DEINIT_API == STD_ON)) || (ADC_DUAL_CLOCK_MODE == STD_ON) \
      || (ADC_CALIBRATION == STD_ON) || (ADC_SET_HW_UNIT_POWER_MODE_API == STD_ON) \
      || (ADC_ENABLE_TEMPSENSE_API == STD_ON) \
    )
                                                            ...
                                                            uint8 UserId)
{
    ...
    {
        ValidUserId = Adc_CheckCurrentUserId(Adc_apxCfgPtr[UserId], UserId);
...
#if (ADC_PRECOMPILE_SUPPORT == STD_ON)
                ValidUserId = Adc_CheckCurrentUserId(Adc_ConfigVariantPredefined[UserId], UserIdTemp);
#else
                ValidUserId = Adc_CheckCurrentUserId(ConfigPtr, UserId);
#endif /* (ADC_PRECOMPILE_SUPPORT == STD_ON) */
                if ((Std_ReturnType)E_OK == ValidUserId)
                {
#if (ADC_PRECOMPILE_SUPPORT == STD_ON)
...
void Adc_DeInit(void)
{
    ...
    {
#endif /* ADC_VALIDATE_GLOBAL_CALL == STD_ON */
        ValidUserId = Adc_CheckCurrentUserId(Adc_apxCfgPtr[UserId], UserIdTemp);
        if ((Std_ReturnType)E_OK == ValidUserId)
        {
