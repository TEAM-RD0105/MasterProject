...
#define MEM_43_INFLS_IPW_SW_MAJOR_VERSION_C                    6
                                               ...
                                               )
{
    ...
    /* The Utest sector and block 0 grouped in the same partition. So, when accessing at the same time, RWW error will occur => use loadAc
       BlockAc default is C40_IP_CODE_BLOCK_0
    */
   if (((Mem_43_INFLS_BlockType)C40_IP_BLOCK_INVALID != JobBlockSector) && \
       ((Mem_43_INFLS_BlockType)C40_IP_BLOCK_INVALID != BlockAc) && \
       ((JobBlockSector == BlockAc) || (((Mem_43_INFLS_BlockType)C40_IP_BLOCK_UTEST == JobBlockSector) && ((Mem_43_INFLS_BlockType)C40_IP_CODE_BLOCK_0 == BlockAc))
       )
      )
    {
