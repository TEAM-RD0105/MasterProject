...
#define EMIOS_ICU_IP_IRQ_SW_MAJOR_VERSION_C                6
                                                          ...
                                                          )
{
    ...
    if (EMIOS_ICU_IP_CHANNEL_7 >= hwChannel)
    {
        ...
        if (EMIOS_ICU_IP_MCB_INT_CLOCK_U32 == Emios_Icu_Ip_GetChannelClockMode(instance, EMIOS_ICU_IP_CHANNEL_0))
        {
    ...
    else if (EMIOS_ICU_IP_CHANNEL_15 >= hwChannel)
    {
        /* eMIOS Counter Bus_C Channel (EMIOS_ICU_IP_CHANNEL_8) is in Modulous counter buffer or
         Modulous counter mode */
        if (EMIOS_ICU_IP_MCB_INT_CLOCK_U32 == Emios_Icu_Ip_GetChannelClockMode(instance, EMIOS_ICU_IP_CHANNEL_8))
        {
    ...
    else if (EMIOS_ICU_IP_CHANNEL_23 >= hwChannel )
    {
        /* eMIOS Counter Bus_D Channel (EMIOS_ICU_IP_CHANNEL_16) is in Modulous counter buffer or
         Modulous counter mode */
        if (EMIOS_ICU_IP_MCB_INT_CLOCK_U32 == Emios_Icu_Ip_GetChannelClockMode(instance, EMIOS_ICU_IP_CHANNEL_16))
        {
    ...
    if (u8ChIndex != EMIOS_ICU_IP_CHANNEL_NOT_USED)
    {
        ...
        else if ((uint8)EMIOS_ICU_IP_MASTERBUS_CHANNEL_USED == u8ChIndex)
        {
            if(((0U == (channel & (uint8)EMIOS_ICU_IP_CB_DIVERSE)) || \
                (EMIOS_ICU_IP_CHANNEL_22 == channel) || \
