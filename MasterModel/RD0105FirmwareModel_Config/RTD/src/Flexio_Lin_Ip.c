...
#define FLEXIO_LIN_IP_SW_MAJOR_VERSION_C               6
...
#endif /* (STD_ON == FLEXIO_LIN_IP_DEV_ERROR_DETECT) */
#if (FLEXIO_LIN_IP_HAS_DMA_ENABLED == STD_ON)
    #if ((FLEXIO_LIN_IP_AR_RELEASE_MAJOR_VERSION_C != DMA_IP_AR_RELEASE_MAJOR_VERSION) || \
        (FLEXIO_LIN_IP_AR_RELEASE_MINOR_VERSION_C != DMA_IP_AR_RELEASE_MINOR_VERSION) \
        )
        #error "AutoSar Version Numbers of Flexio_Lin_Ip.c and Dma_Ip.h are different"
    #endif
#endif
#endif
...
#if (FLEXIO_LIN_IP_HAS_DMA_ENABLED == STD_ON)
    #define FLEXIO_LIN_DMA_CONFIG_LIST_DIMENSION             (10U)
    #define FLEXIO_LIN_DMA_LEAST_CONFIG_LIST_DIMENSION       (2U)
    /* @brief Address of the least significant byte or word in a 32-bit register (depends on endianness) */
    #define FLEXIO_LIN_IP_LSBW_ADDR(reg)  ((uint32)(&(reg)))
#endif
/*==================================================================================================
*                                         LOCAL CONSTANTS
==================================================================================================*/
...
#if (FLEXIO_LIN_IP_HAS_DMA_ENABLED == STD_ON)
static Flexio_Lin_Ip_StatusType Flexio_Lin_Ip_StartReceiveResponseUsingDma(uint8 Channel, uint8 * RxBuff, uint32 RxSize);
static void Flexio_Lin_Ip_CompleteRxOperationUsingDma(const uint8 Channel);
...
/*FUNCTION**********************************************************************
 *
 * Function Name : Flexio_Lin_Ip_StartReceiveResponseUsingDma
...
static Flexio_Lin_Ip_StatusType Flexio_Lin_Ip_StartReceiveResponseUsingDma(uint8 Channel, uint8 * RxBuff, uint32 RxSize)
{
    ...
    FLEXIO_Type * Base;
    Dma_Ip_LogicChannelTransferListType DmaTransferList[FLEXIO_LIN_DMA_CONFIG_LIST_DIMENSION];
    Dma_Ip_ReturnType DmaReturnStatus;

    LinCurrentState = (Flexio_Lin_Ip_StateStructType *)Flexio_Lin_Ip_apxStateStructureArray[Channel];
    UserConfig = Flexio_Lin_Ip_apxUserConfigs[Channel];
    Base = Flexio_Lin_Ip_apxBases[UserConfig->FlexioHwInstance];

    /* Set the state node and mark the channels as busy */
    ...
    /* Enable error interrupt and disable Rx interrupt */
    Flexio_Mcl_Ip_SetShifterErrorInterrupt(Base, (uint8)(1U << UserConfig->RxShifterId), TRUE);
    Flexio_Mcl_Ip_SetShifterInterrupt(Base, (uint8)(1U << UserConfig->RxShifterId), FALSE);

    /* Set up parameters for Dma_Ip_LogicChannelTransferListType */
    DmaTransferList[0].Param = DMA_IP_CH_SET_SOURCE_ADDRESS;
    DmaTransferList[0].Value = FLEXIO_LIN_IP_LSBW_ADDR(Base->SHIFTBUF[UserConfig->RxShifterId]) + 3U;
    ...
    LinCurrentState->SduBuf = RxBuff;
    LinCurrentState->CntByte = 0U;

    /* Configure the transfer control descriptor for the DMA channel */
    DmaReturnStatus = Dma_Ip_SetLogicChannelTransferList(UserConfig->RxDMAChannel, DmaTransferList, FLEXIO_LIN_DMA_CONFIG_LIST_DIMENSION);
#if (FLEXIO_LIN_IP_DEV_ERROR_DETECT == STD_ON)    
    FLEXIO_LIN_IP_DEV_ASSERT(DMA_IP_STATUS_SUCCESS == DmaReturnStatus);
#endif

    /* Start the DMA channel */
    DmaReturnStatus = Dma_Ip_SetLogicChannelCommand(UserConfig->RxDMAChannel, DMA_IP_CH_SET_HARDWARE_REQUEST);
#if (FLEXIO_LIN_IP_DEV_ERROR_DETECT == STD_ON) 
    FLEXIO_LIN_IP_DEV_ASSERT(DMA_IP_STATUS_SUCCESS == DmaReturnStatus);
#endif
    (void)DmaReturnStatus;
    /* Enable FlexIO DMA requests */
    Flexio_Mcl_Ip_SetShifterDMARequest(Base, (uint8)(1U << UserConfig->RxShifterId), TRUE);

    return FLEXIO_LIN_IP_STATUS_SUCCESS;
}
/*FUNCTION**********************************************************************
 *
 * Function Name : Flexio_Lin_Ip_CompleteRxOperationUsingDma
 * Description   : Complete operation of the FLEXIO receiver using DMA.
 *
 *END**************************************************************************/
static void Flexio_Lin_Ip_CompleteRxOperationUsingDma(const uint8 Channel)
{
    ...
    Flexio_Mcl_Ip_SetShifterDMARequest(Base, (uint8)(1U << UserConfig->RxShifterId), FALSE);
    ...
    if (FLEXIO_LIN_IP_STATUS_DMA_ERROR != LinCurrentState->CurrentEventId)
    {
        ...
        if (TempByte != Flexio_Lin_Ip_au8SduBuffer[Channel][LinCurrentState->Dl])
        {
        ...
        else
        {
            ...
            LinCurrentState->SduBuf = &Flexio_Lin_Ip_au8SduBuffer[Channel][0u];
...
static void Flexio_Lin_Ip_RxPidByteProcess(const uint8 Channel, const uint8 Data)
{
    ...
    else
    {
        ...
        {
            if (UserConfig->TransferType == FLEXIO_LIN_IP_USING_INTERRUPTS)
            {
            ...
            {
                /* LinCurrentState->Dl + 1 for checksum data */
                Flexio_Lin_Ip_StartReceiveResponseUsingDma(Channel,Flexio_Lin_Ip_au8SduBuffer[Channel], LinCurrentState->Dl + 1UL);
                #if (FLEXIO_LIN_IP_FRAME_TIMEOUT_DISABLE == STD_OFF)
                #ifdef FLEXIO_LIN_IP_START_TIMER_NOTIFICATION
                LinCurrentState->TimerCounting = TRUE;
                /* Call notification to start timer for the checking response reception timeout */
...
/*FUNCTION**********************************************************************
 *
 * Function Name : Flexio_Lin_Ip_CompleteReceiveUsingDma
 ...
 *END**************************************************************************/
/**
* @implements Flexio_Lin_Ip_CompleteReceiveUsingDma_Activity
*/
void Flexio_Lin_Ip_CompleteReceiveUsingDma(uint8 Channel)
{
#if (FLEXIO_LIN_IP_DEV_ERROR_DETECT == STD_ON)
    FLEXIO_LIN_IP_DEV_ASSERT(Channel < FLEXIO_LIN_IP_NUMBER_OF_FLEXIO_SHIFTERS);
#endif
    Flexio_Lin_Ip_StateStructType *LinCurrentState;
    const Flexio_Lin_Ip_UserConfigType * UserCfg;
    Dma_Ip_ReturnType DmaReturnStatus;
    Dma_Ip_LogicChannelStatusType DmaStatus;

    LinCurrentState = (Flexio_Lin_Ip_StateStructType *)Flexio_Lin_Ip_apxStateStructureArray[Channel];
    UserCfg = Flexio_Lin_Ip_apxUserConfigs[Channel];
    DmaStatus.ChStateValue = DMA_IP_CH_ERROR_STATE;

    /* Get Dma Ip Logic Channel Status */
    (void)Dma_Ip_GetLogicChannelStatus(UserCfg->RxDMAChannel, &DmaStatus);
    if (DMA_IP_CH_ERROR_STATE == DmaStatus.ChStateValue)
    {
        /* Reset the Dma Channel Error status. */
        DmaReturnStatus = Dma_Ip_SetLogicChannelCommand(UserCfg->RxDMAChannel, DMA_IP_CH_CLEAR_ERROR);
#if (FLEXIO_LIN_IP_DEV_ERROR_DETECT == STD_ON)
        FLEXIO_LIN_IP_DEV_ASSERT(DMA_IP_STATUS_SUCCESS == DmaReturnStatus);
#endif
        /* Update event id */
        LinCurrentState->CurrentEventId = FLEXIO_LIN_IP_STATUS_DMA_ERROR;
    }
    (void)DmaReturnStatus;
    /* Operation for complete receive data */
    Flexio_Lin_Ip_CompleteRxOperationUsingDma(Channel);
}
#endif
/*FUNCTION**********************************************************************
 *
...
#if (FLEXIO_LIN_IP_HAS_DMA_ENABLED == STD_ON)
    if (UserConfig->TransferType == FLEXIO_LIN_IP_USING_DMA)
    {
        Flexio_Mcl_Ip_SetShifterDMARequest(Base, (uint8)(1U << UserConfig->RxShifterId), FALSE);
    }  
#endif
...
void Flexio_Lin_Ip_TimerExpiredService(const uint8 Channel)
{
    ...
    UserConfig = Flexio_Lin_Ip_apxUserConfigs[Channel]; 
#if (FLEXIO_LIN_IP_HAS_DMA_ENABLED == STD_ON)
    FLEXIO_Type *Base = Flexio_Lin_Ip_apxBases[UserConfig->FlexioHwInstance];
    ...
    switch(LinCurrentState->CurrentNodeState)
    {
        ...
        case FLEXIO_LIN_IP_NODE_STATE_RECV_DATA_COMPLETED:
            ...
            */
            ...
            /* CntByte is used in Flexio_Lin_Ip_GetStatusFromTimeoutError function to report error */
            LinCurrentState->CntByte = LinCurrentState->Dl + 1UL - RemainingByte;
#endif
